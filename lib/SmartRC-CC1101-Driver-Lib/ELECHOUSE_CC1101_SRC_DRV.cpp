...
byte gdo_set=0;
bool spi = 0;
// Tracks whether SPI.begin() has already been called for the current
// SCK/MISO/MOSI/SS pin set. The original driver called SPI.begin()/end()
// around every single register transfer (SpiStart()/SpiEnd() are invoked
// once per SpiWriteReg/SpiReadReg/SpiStrobe/etc. call). On current ESP32
// cores that begin/end churn corrupts the SPI peripheral state and hangs
// the hardware on the next transfer, tripping the task watchdog. Keeping
// the bus open across transfers (only re-begun when the pin set actually
// changes, see setSpiPin()) avoids that while preserving the driver's
// existing call structure.
static bool spiBusStarted = false;
bool ccmode = 0;
float MHz = 433.92;
byte m4RxBw = 0;
byte m4DaRa;
...
{
  // Intentionally left as a no-op: this used to call SPI.end() on every
  // single register transfer (see SpiStart() above), which is what
  // caused the hang. The bus is released explicitly in setSpiPin() when
  // the pin assignment actually changes.
}
/****************************************************************
...
void ELECHOUSE_CC1101::Init(void)
{
  ...
  digitalWrite(SS_PIN, HIGH);
  // NOTE: SCK_PIN/MOSI_PIN are bound to the hardware SPI peripheral by
  // SPI.begin() inside SpiStart(). Calling digitalWrite() on them here
  // (as the upstream driver does) is rejected by the ESP32 core's pin
  // ownership check and corrupts the SPI peripheral state, causing the
  // next SPI.transfer() in RegConfigSettings() to hang forever and trip
  // the task watchdog. SPI.begin() already drives these lines to their
  // correct idle levels, so no manual write is needed.
  Reset();                    //CC1101 reset
...
void ELECHOUSE_CC1101::setSpiPin(byte sck, byte miso, byte mosi, byte ss){
  if (spiBusStarted && (SCK_PIN != sck || MISO_PIN != miso || MOSI_PIN != mosi || SS_PIN != ss)) {
    // Pin assignment changed: release the previously opened bus so the
    // next SpiStart() re-begins SPI on the new pins instead of skipping
    // initialization because a (now stale) session looks already open.
    SPI.end();
    spiBusStarted = false;
  }
  spi = 1;
